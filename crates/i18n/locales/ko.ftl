...
add_local_library = 로컬 라이브러리 추가
local_library_name = 라이브러리 이름
local_library_name_required = 라이브러리 이름은 필수입니다
local_library_folder_required = 폴더를 하나 이상 선택하세요
service = 서비스: { $name }
connected = ● 연결됨
disconnected = ● 연결 끊김
connecting = ● 연결 중…
...
remote_music_folders = 음악 폴더
parent_folder = 위로
use_this_folder = 이 폴더 추가
no_subfolders = 하위 폴더 없음
loading_folders = 폴더 불러오는 중…
folder_browse_failed = 폴더를 불러올 수 없습니다: { $error }
add_server = 서버 추가
connect_to_lastfm = Last.fm에 연결
connect_to_librefm = Libre.fm에 연결
lastfm_connected = Last.fm 연결됨
librefm_connected = Libre.fm 연결됨
...
cover_art_background = 커버 아트를 배경으로
cover_art_darkening = 커버 아트 어둡게
cover_art_blur = 커버 아트 흐림
custom_background = 사용자 지정 배경
choose_image = 이미지 선택
custom_font = 사용자 지정 글꼴
choose_font = 글꼴 선택
live_theme = Matugen / Pywal(실시간)
live_theme_file = 팔레트 파일
choose_palette = 팔레트 선택
jump_to_current_song = 현재 곡으로 이동
hide_side_panel = 사이드 패널 숨기기
...
start_radio = 라디오 시작
radio_starting = 라디오 시작 중…
radio_empty = 유사한 곡을 찾을 수 없습니다
radio_failed = 라디오를 시작할 수 없습니다
...
quick_search_placeholder = 입력하여 검색, Enter로 재생…
...
lastfm = Last.fm
librefm = Libre.fm
enable_tracing = 성능 추적 활성화
tracing_warning = 오버헤드가 늘고 추적 파일이 커집니다. 다시 시작한 후 적용됩니다.
export_logs = 로그 내보내기
...
lyrics_depth_blur_strength = 가사 흐림 강도
...
sort_field_genre = 장르
sort_field_album = 앨범
sort_field_duration = 재생 시간
sort_field_date_added = 추가된 날짜
sort_field_name = 이름
sort_field_tracks = 트랙
sort_field_albums = 앨범
view_grid = 격자 보기
view_list = 목록 보기
spotify_browser = Spotify 재생 브라우저
spotify_browser_auto = 자동
spotify_connect_device = Spotify 재생 기기
spotify_connect_device_other = 다른 기기
spotify_connect_device_this = 이 앱
