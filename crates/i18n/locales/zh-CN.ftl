...
add_local_library = 添加本地音乐库
local_library_name = 音乐库名称
local_library_name_required = 必须填写音乐库名称
local_library_folder_required = 请至少选择一个文件夹
service = 服务：{ $name }
connected = ● 已连接
disconnected = ● 未连接
connecting = ● 连接中…
...
remote_music_folders = 音乐文件夹
parent_folder = 上一级
use_this_folder = 添加此文件夹
no_subfolders = 没有子文件夹
loading_folders = 正在加载文件夹…
folder_browse_failed = 无法列出文件夹：{ $error }
add_server = 添加服务器
connect_to_lastfm = 连接到 Last.fm
connect_to_librefm = 连接到 Libre.fm
lastfm_connected = 已连接到 Last.fm
librefm_connected = 已连接到 Libre.fm
...
custom_font = 自定义字体
choose_font = 选择字体
live_theme = Matugen / Pywal（实时）
live_theme_file = 调色板文件
choose_palette = 选择调色板
jump_to_current_song = 跳转到当前歌曲
hide_side_panel = 隐藏侧边栏
show_side_panel = 显示侧边栏
...
quick_search_placeholder = 输入以搜索，回车播放…
...
lastfm = Last.fm
librefm = Libre.fm
enable_tracing = 启用性能追踪
tracing_warning = 会增加开销并使追踪文件变大；重启后生效。
export_logs = 导出日志
...
sort_field_genre = 流派
sort_field_album = 专辑
sort_field_duration = 时长
sort_field_date_added = 添加日期
sort_field_name = 名称
sort_field_tracks = 曲目
sort_field_albums = 专辑
view_grid = 网格视图
view_list = 列表视图
spotify_browser = Spotify 播放浏览器
spotify_browser_auto = 自动
spotify_connect_device = Spotify 播放设备
spotify_connect_device_other = 其他设备
spotify_connect_device_this = 此应用
spotify_play_on = 播放设备
spotify_this_app = kopuz（此应用）
settings_managed_notice = 部分设置由系统配置（Nix/hjem）管理，无法在此更改。
setting_managed_by_system = 由系统配置管理
