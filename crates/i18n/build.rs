...
fn main() {
    ...
    let mut languages: Vec<(String, String, String)> = vec![];
    ...
    for entry in fs::read_dir(&locales_dir).expect("locales directory not found") {
        ...
        if path.extension().is_some_and(|e| e == "ftl") {
            ...
            languages.push((code, name, content));
        }
    }
    ...
    for (lang_code, _, content) in &languages {
        code.push_str(&format!("        \"{lang_code}\" => Some({content:?}),\n"));
    }
